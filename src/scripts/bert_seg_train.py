...
df = pd.read_excel(script_dir + '/data/segmenter/dataset_finale_train.xlsx', converters={'Stralci': literal_eval, 'Repertori': literal_eval})
df_s2 = pd.read_csv(script_dir + '/data/segmenter/ie_s2_hyperion_train.csv', converters={'Stralci': literal_eval, 'Repertori': literal_eval})
test_df = pd.read_csv(script_dir + '/data/segmenter/ie_s2_hyperion_test.csv', converters={'Stralci': literal_eval, 'Repertori': literal_eval})
...
run['val/norm_metrics'] = out['normalized_metrics']
run['val/metrics'] = out['metrics']
run['val/loss'] = out['loss']
run['val/predicted_spans'] = out['predicted_spans']


out = trainer.test(model, test_dataset, config['batch_size'], torch.nn.CrossEntropyLoss(weight = torch.Tensor(config['loss_weights'])))
run['test/norm_metrics'] = out['normalized_metrics']
run['test/metrics'] = out['metrics']
run['test/loss'] = out['loss']
run['test/predicted_spans'] = out['predicted_spans']
